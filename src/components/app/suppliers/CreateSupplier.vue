...
import type { NewSupplierDto } from "@/interfaces/suppliers";
...
const name: Ref<NewSupplierDto["name"] | undefined> = ref();
const email: Ref<NewSupplierDto["email"] | undefined> = ref();
const phone: Ref<NewSupplierDto["phone"] | undefined> = ref();
...
const onFormSubmitHandler = async (event: Event) => {
  ...
  try {
    ...
    if (valid) {
      ...
      if (success) {
        toastRefInfo.value?.toastText("The supplier was created successfully!");
        toastRefInfo.value?.toastElapsedDuration("Just now");
        toastRefInfo.value?.toastHeading("Create Suppliers");
        toastRefInfo.value?.toastName("create-supplier");
    ...
    toastRefDanger.value?.toastName("supplier-create-error");
